...
router.post("/:order_id", multer.single("image"), async(req, res) => {
    try {
        await controller.add(req.params.order_id, req.file);
        response.success(res, 201, "Creado exitosamente");
    } catch (error) {
        console.log(error)
        response.error(res, 400, "Algo salió mal");
    }
...
router.patch("/:id", async(req, res) => {
    try {
        ...
        response.success(res, 200, "Actualizado correctamente");
    } catch (error) {
        response.error(res, 400, "Algo salió mal");
    }
})
